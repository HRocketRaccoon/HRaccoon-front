...
const userNo = ref(1) // 사용자 번호를 임의로 설정
...
const isSeatSelectionDisabled = seatNumber => {
  return selected.value.length > 0 || checked.value.includes(seatNumber)
}

const fetchSeatData = async () => {
  try {
    const response = await api.get(`/seat/office/${seatOffice.value}/${floor.value}`)

    if (response.data && response.data.status === 'success' && response.data.data.length > 0) {
      // seatStatusYn이 true인 seatNo만 checked 배열에 추가
      checked.value = response.data.data.filter(seat => seat.seatStatusYn).map(seat => seat.seatNo)
    } else {
      checked.value = []
    }

    console.log('seatOffice:', seatOffice.value)
...
const confirmSelection = async () => {
  ...
  try {
    const response = await api.post(`/seat/availableseats/${seatOffice.value}/select/${seatNo.value}/${userNo.value}`)
